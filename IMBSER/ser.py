...
# Assuming other auxiliary functions like `fusionDecisionTree` are defined elsewhere
...
def SER(node, dTree, X_target_node, y_target_node, original_ser=True, no_red_on_cl=False,
        cl_no_red=None, no_ext_on_cl=False, cl_no_ext=None, ext_cond=None, leaf_loss_quantify=False, leaf_loss_threshold=None, coeffs=None, root_source_values=None, Nkmin=None, ccp_alpha = None):
    ...
    if dTree.tree_.feature[node] == -2:
        ...
        if y_target_node.size > 0 and len(set(list(y_target_node))) > 1:
            ...
            DT_to_add = DecisionTreeClassifier(max_depth=2)
...
def SERRR(node, dTree, X_target_node, y_target_node, original_ser=True, ccp_alpha=None):
    ### VALUES UPDATE ###
    class_weights = class_weight.compute_class_weight('balanced', classes=np.unique(y_target_node), y=y_target_node)
    class_weights_dict = dict(enumerate(class_weights))

    if dTree.tree_.feature[node] == -2:
        if y_target_node.size > 0 and len(set(y_target_node)) > 1:
            DT_to_add = DecisionTreeClassifier(max_depth=3, ccp_alpha=ccp_alpha if ccp_alpha is not None else 0.01, class_weight=class_weights_dict)
            DT_to_add.fit(X_target_node, y_target_node)
            fusionDecisionTree(dTree, node, DT_to_add)
        return node, False

    ### Recursively expand tree ###
    left_indices = X_target_node[:, dTree.tree_.feature[node]] <= dTree.tree_.threshold[node]
    right_indices = ~left_indices

    if any(left_indices):
        new_node_left, _ = SER(dTree.tree_.children_left[node], dTree, X_target_node[left_indices], y_target_node[left_indices], original_ser)
    if any(right_indices):
        new_node_right, _ = SER(dTree.tree_.children_right[node], dTree, X_target_node[right_indices], y_target_node[right_indices], original_ser)
