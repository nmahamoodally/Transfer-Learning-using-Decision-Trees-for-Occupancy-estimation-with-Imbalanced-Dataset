...
def threshold_selection(Q_source_parent,
    ...
    # print("use divergence ?: ", use_divergence)
...
def classify(tree, x):
    # Using decision path to determine the node index
    node_indicator = tree.decision_path(x.reshape(1, -1))
    # Convert the sparse matrix to dense format and get the node indices
    node_index = node_indicator.indices[node_indicator.indptr[0]:node_indicator.indptr[1]]
    
    # Check if the last node is a leaf
    is_leaf = tree.tree_.children_left[node_index[-1]] == tree.tree_.children_right[node_index[-1]] == -1
    if is_leaf:
        # Return the probability distribution for classes
        return tree.tree_.value[node_index[-1]].squeeze()
    else:
        raise Exception("The classify function expected a leaf node but got a non-leaf node.")
        
def tree_posterior(tree, x):
    """ Returns the posterior probability distribution of the classes for a single input x using one decision tree. """
...
def forest_posterior(RF, x):
    ...
    # Get the posterior for each tree and infer the number of classes
    for tree in RF:
        p = classify(tree, x)
        Pt.append(p)
